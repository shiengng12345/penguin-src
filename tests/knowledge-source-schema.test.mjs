...
test("v9-labelled database with indexed data requires a fresh current-schema generation", () => {
  ...
  assert.throws(
    () => KnowledgeStore.open({ dbPath: join(dir, "knowledge.db"), ledgerPath: join(dir, "ledger.jsonl") }),
    (error) => error?.code === "REINDEX_REQUIRED"
      && error?.storedVersion === 9
      && error?.supportedVersion === 18,
  );
});
