...
test("real stable launcher observes versioned runtime updates as outdated", async () => {
  const home = mkdtempSync(join(tmpdir(), "pgv-launcher-e2e-"));
  const runtimeRoot = join(home, "runtimes");
  ...
  try {
    ...
    const child = spawn(process.execPath, [fileURLToPath(launcher)], {
      cwd: process.cwd(),
