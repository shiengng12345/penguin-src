...
export function WikiSearchPage() {
  ...
  return <div className="flex min-h-0 flex-1 flex-col bg-background">
    <div className="flex items-center gap-2 border-b border-border p-4">
      ...
      <input aria-label="筛选仓库" value={repoFilter} onChange={(event) => setRepoFilter(event.target.value)} placeholder="repo" className="w-24 rounded border border-border bg-card px-2 py-1 text-xs text-foreground" />
      <input aria-label="筛选分支" value={branchFilter} onChange={(event) => setBranchFilter(event.target.value)} placeholder="branch" className="w-20 rounded border border-border bg-card px-2 py-1 text-xs text-foreground" />
      <input aria-label="筛选 snapshot" value={snapshotFilter} onChange={(event) => setSnapshotFilter(event.target.value)} placeholder="snapshot" className="w-24 rounded border border-border bg-card px-2 py-1 text-xs text-foreground" />
      <input aria-label="筛选路径" value={pathFilter} onChange={(event) => setPathFilter(event.target.value)} placeholder="path" className="w-28 rounded border border-border bg-card px-2 py-1 text-xs text-foreground" />
      <input aria-label="筛选语言" value={languageFilter} onChange={(event) => setLanguageFilter(event.target.value)} placeholder="language" className="w-20 rounded border border-border bg-card px-2 py-1 text-xs text-foreground" />
      <input aria-label="筛选 kind" value={kindFilter} onChange={(event) => setKindFilter(event.target.value)} placeholder="kind" className="w-16 rounded border border-border bg-card px-2 py-1 text-xs text-foreground" />
      <input aria-label="预览行数" type="number" min={0} max={100} value={previewLines} onChange={(event) => { const value = Math.max(0, Math.min(100, Number(event.target.value) || 0)); setPreviewLines(value); setPersistedValue(APP_VALUE_KEYS.wikiPreviewLines, String(value)); }} className="w-14 rounded border border-border bg-card px-2 py-1 text-xs text-foreground" title="预览前后行数" />
      <select aria-label="筛选 evidence 状态" value={evidenceFilter} onChange={(event) => setEvidenceFilter(event.target.value)} className="rounded border border-border bg-card px-2 py-1 text-xs text-foreground"><option value="all">evidence</option><option value="verified">verified</option><option value="observed">observed</option><option value="inference">inference</option></select>
      <input aria-label="保存查询名称" value={savedName} onChange={(event) => setSavedName(event.target.value)} placeholder="保存名" className="w-20 rounded border border-border bg-card px-2 py-1 text-xs text-foreground" />
      <button type="button" aria-label="保存当前查询" disabled={!savedName.trim() || !query.trim()} onClick={() => void saveCurrentQuery()} className="rounded border border-border p-1 text-muted-foreground disabled:opacity-40"><Bookmark className="h-3.5 w-3.5" /></button>
    </div>
    ...
    {busy && <div className="flex items-center gap-2 px-4 py-3 text-xs text-muted-foreground"><Loader2 className="h-3.5 w-3.5 animate-spin" /> 搜索中…</div>}
    {!busy && response && <div ref={resultViewport} tabIndex={0} onScroll={(event) => setResultScrollTop(event.currentTarget.scrollTop)} onKeyDown={(event) => {
      ...
      {orderedSavedQueries.length > 0 && <div className="mb-3 flex flex-wrap gap-1.5" aria-label="Knowledge 已保存查询">{orderedSavedQueries.map((saved) => <span key={saved.id} className="inline-flex items-center rounded border border-border"><button type="button" onClick={() => { void knowledgeSavedQueryRun(saved.name).then(setResponse); }} className="px-2 py-1 text-[11px] text-muted-foreground hover:text-cyan-200"><Bookmark className="mr-1 inline h-3 w-3" />{saved.name}</button><button type="button" aria-label={`${pinnedSavedQueries.includes(saved.id) ? "取消置顶" : "置顶"} ${saved.name}`} onClick={() => togglePinnedSavedQuery(saved.id)} className="border-l border-border px-1.5 py-1 text-[10px] text-muted-foreground hover:text-amber-300">{pinnedSavedQueries.includes(saved.id) ? "★" : "☆"}</button></span>)}</div>}
      {recentQueries.length > 0 && <div className="mb-3 flex flex-wrap items-center gap-1.5 text-[11px] text-muted-foreground"><span>最近查询</span>{recentQueries.map((recent) => <button type="button" key={recent} onClick={() => setQuery(recent)} className="rounded border border-border px-2 py-1 text-muted-foreground hover:border-cyan-500/40 hover:text-cyan-200">{recent}</button>)}</div>}
      ...
      {visibleHits.length === 0 ? <div className="rounded border border-yellow-500/20 bg-yellow-500/5 p-4 text-sm text-yellow-200">
        ...
        {visibleHits.slice(virtualWindow.start, virtualWindow.end).map((hit, offset) => { const index = virtualWindow.start + offset; return <div key={hit.hitId} style={{ height: ROW_HEIGHT }} className="pb-2"><article role="button" tabIndex={0} aria-selected={selectedHitIndex === index} onClick={() => { setSelectedHitIndex(index); void openContext(hit); }} onFocus={() => setSelectedHitIndex(index)} onKeyDown={(event) => { if (event.key === "Enter" && event.metaKey) { event.preventDefault(); void openCodeLocation(hit); } else if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedHitIndex(index); void openContext(hit); } }} className={`flex h-full flex-col overflow-hidden cursor-pointer rounded-lg border bg-background/35 p-3 ${selectedHitIndex === index ? "border-cyan-400/80 ring-1 ring-cyan-400/30" : hit.lane === "semantic" ? "border-violet-500/30" : "border-border hover:border-cyan-500/40"}`}>
          ...
          <div className="mt-2 shrink-0 text-[10px] text-muted-foreground">{hit.rankReasons.join(" · ")}</div>
        </article></div>; })}
        <div aria-hidden="true" style={{ height: `${virtualWindow.bottom}px` }} />
        </div>}
      {contextBusy && <div className="mt-4 text-xs text-muted-foreground">加载知识上下文…</div>}
      {hitPreview && <section className="mt-4 rounded-lg border border-border bg-background/70 p-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground"><span className="text-foreground">完整 excerpt</span><span>{hitPreview.evidence[0]?.status}</span><span className="rounded border border-border px-1.5 py-0.5">{hitPreview.locator.revisionKind === "working_tree" ? "working tree" : `只读 commit: ${hitPreview.locator.commitSha ?? hitPreview.locator.revisionId}`}</span></div>
        {hitPreview.snippet && <div className="mt-2 max-h-64 overflow-auto font-mono text-xs text-foreground">{hitPreview.snippet.split("\n").map((line, index) => <div key={`${hitPreview.hitId}-line-${index}`} className="flex min-w-max"><button type="button" className="mr-3 w-10 shrink-0 select-none text-right text-muted-foreground hover:text-cyan-300" onClick={() => { const hit = visibleHits.find((item) => item.hitId === hitPreview.hitId); if (hit) void openCodeLocation({ ...hit, locator: { ...hit.locator, startLine: (hitPreview.locator.startLine ?? 1) + index } }); }}>{(hitPreview.locator.startLine ?? 1) + index}</button><span className="whitespace-pre">{line}</span></div>)}</div>}
      ...
      {!scopeBlock && contextPack && <section className="mt-4 rounded-lg border border-cyan-500/30 bg-cyan-950/10 p-4">
        ...
        {contextPack.focus ? <>
          <div className="mt-1 text-sm font-semibold text-foreground">{contextPack.focus.title}</div>
          <div className="mt-1 font-mono text-[11px] text-muted-foreground">{contextPack.focus.filePath ?? contextPack.target}</div>
          {contextPack.focus.source && <pre className="mt-2 max-h-48 overflow-auto whitespace-pre-wrap text-xs text-foreground">{contextPack.focus.source}</pre>}
        ...
        <div className="mt-3 grid gap-3 text-xs sm:grid-cols-2">
          {([["Callers", contextPack.callers], ["Callees", contextPack.calls], ["Tests", contextPack.tests], ["Routes", contextPack.routes.map((route) => ({ title: route.route, nodeId: route.via }))]] as const).map(([label, items]) => <div key={label}><div className="text-muted-foreground">{label}</div>{items.slice(0, 8).map((item) => <div key={`${label}-${item.nodeId}-${item.title}`} className="truncate text-foreground">{item.title}</div>)}</div>)}
        </div>
      ...
      {evidence.length > 0 && <section className="mt-4 rounded-lg border border-emerald-500/30 bg-emerald-950/10 p-4"><div className="text-xs uppercase tracking-wide text-emerald-300"><ClipboardList className="mr-1 inline h-3.5 w-3.5" />相关 Evidence</div><div className="mt-2 space-y-1 text-xs text-foreground">{evidence.map((item) => <div key={item.slug}>{item.title} · {item.status} · {item.targetId}</div>)}</div></section>}
      {noteLinks.length > 0 && <section className="mt-4 rounded-lg border border-amber-500/30 bg-amber-950/10 p-4"><div className="text-xs uppercase tracking-wide text-amber-300">Note backlinks / mentions</div><div className="mt-2 space-y-1 text-xs text-foreground">{noteLinks.map((item) => <div key={item.nodeId}>{item.title} · {item.nodeType}</div>)}</div></section>}
      {graphView && <section className="mt-4 rounded-lg border border-violet-500/30 bg-violet-950/10 p-4">
        ...
        <div className="mt-2 text-xs text-foreground">{graphView.nodes.length} nodes · {graphView.edges.length} edges</div>
        <div className="mt-2 flex flex-wrap gap-1.5">{graphView.nodes.slice(0, 30).map((node) => <button type="button" key={node.nodeId} onClick={() => focusGraphNode(node)} className={`rounded bg-card px-2 py-1 text-[11px] ${selectedGraphNode?.nodeId === node.nodeId ? "text-cyan-200 ring-1 ring-cyan-400/50" : "text-foreground"}`}>{node.title} <span className="text-muted-foreground">{node.nodeType}</span></button>)}</div>
        {selectedGraphNode && <div className="mt-3 flex flex-wrap items-center gap-2 rounded border border-cyan-500/20 px-2 py-2 text-xs text-foreground"><span>已选：{selectedGraphNode.title}</span><button type="button" onClick={() => setQuery(selectedGraphNode.title)} className="rounded border border-border px-2 py-1 text-cyan-200">送到搜索</button><button type="button" onClick={() => void openGraphNodeContext()} className="rounded border border-border px-2 py-1 text-cyan-200">送到 context</button><button type="button" onClick={exportGraphSelection} className="rounded border border-border px-2 py-1 text-cyan-200">导出 Canvas</button></div>}
      </section>}
      {response.page.nextCursor && <button type="button" onClick={() => void loadMore()} className="mt-4 w-full rounded border border-border px-3 py-2 text-xs text-foreground hover:border-cyan-500/50 hover:text-cyan-200">加载更多结果</button>}
    </div>}
  </div>;
}
