...
export function WikiFlowPane({
  ...
  return (
    <div className="flex-1 overflow-auto p-6">
      ...
      <div className="mb-5 text-xs text-muted-foreground">执行链 — 由静态边推导(handles → calls → reads/writes/throws/uses)</div>
      <div className="space-y-1">
        {flow.steps.map((s, i) => (
          <div key={i} className="flex items-center" style={{ paddingLeft: s.depth * 24 }}>
            {s.via !== "root" && <span className="px-2 font-mono text-[10px] text-muted-foreground">↳ {s.via} →</span>}
            <button type="button" onClick={() => onSelectSymbol(s.nodeId)}
              className={cn("flex items-center gap-2 rounded-lg border px-3 py-2 font-mono text-xs",
                s.depth === 0 ? "border-emerald-500/50 bg-emerald-500/10 text-emerald-100" : "border-border bg-card/50 text-foreground hover:bg-accent")}>
              <span className="h-2 w-2 rounded-full" style={{ background: VIA_COLOR[s.via] ?? KIND_COLOR[s.nodeType] ?? KIND_COLOR.symbol }} />
              {s.title}<span className="text-[9px] uppercase text-muted-foreground">{s.nodeType}</span>
            </button>
          </div>
        ))}
      </div>
