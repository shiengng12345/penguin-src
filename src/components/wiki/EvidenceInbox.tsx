  ...
  return (
    <section className="mt-4 rounded-xl border border-slate-800 bg-slate-950/40 p-4" aria-label="Evidence Inbox">
      <div className="mb-3 flex items-center gap-3">
        ...
        <select value={status} onChange={(e) => setStatus(e.target.value)} className="rounded border border-slate-700 bg-slate-900 px-2 py-1 text-xs text-slate-300"><option value="">all status</option>{["draft", "reviewed", "verified", "resolved", "archived"].map((item) => <option key={item}>{item}</option>)}</select>
      </div>
      {rows.length === 0 ? <div className="text-xs text-slate-600">暂无 evidence notes；完成 SLS capture 后会自动出现。</div> : <div className="space-y-2">{rows.map((row) => <div key={row.slug} className="rounded border border-slate-800/80 px-3 py-2 text-xs"><div className="flex gap-2"><span className="font-semibold text-slate-300">{row.title}</span><span className="text-cyan-300/80">{row.status}</span><span className={row.indexed ? "text-emerald-300/80" : "text-amber-300/80"}>{row.indexed ? "indexed" : "not indexed"}</span></div><div className="mt-1 text-slate-500">{row.environment} · {row.region} · {row.project}/{row.logstore} · {row.targetId} · observations={row.observationCount}</div></div>)}</div>}
    </section>
  );
}
