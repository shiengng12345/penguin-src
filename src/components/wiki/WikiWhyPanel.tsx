  ...
  return (
    <div className="rounded-xl border border-slate-700 bg-slate-900/70 p-3">
        ...
        {NOTE_TYPES.map((t) => (
          <button
            ...
            className={cn(
              ...
              creating.type === t ? "bg-cyan-500/25 text-cyan-100" : "bg-slate-800 text-slate-400 hover:bg-slate-700",
            )}
          >
            {t}
          </button>
      ...
      <input
        ...
        className="w-full rounded-md border border-slate-800 bg-slate-950/70 px-2 py-1.5 text-xs text-slate-100 outline-none placeholder:text-slate-600 focus:border-cyan-500/40"
      />
      <div className="mt-2 flex justify-end gap-1.5">
        <button type="button" onClick={() => onSetCreating(null)} className="rounded-md px-2 py-1 text-[11px] text-slate-400 hover:bg-white/5">取消</button>
        <button
          type="button"
...
export function WikiWhyPanel({
    ...
    return (
      <div className="flex h-full flex-col gap-2 p-3">
        <div className="flex items-center gap-2 border-b border-slate-800 pb-2">
          ...
          <button type="button" onClick={onCancelEdit} className="rounded-md border border-slate-700 px-2 py-1 text-xs hover:bg-white/5">取消</button>
    ...
    return (
      <div className="flex h-full flex-col p-5">
        <div className="mb-5">
          ...
          <h3 className="text-base font-semibold text-slate-100">Why layer</h3>
          ...
          <div className="space-y-2">
            {NOTE_TYPES.slice(0, 4).map((type) => (
              <button
                ...
                className="flex w-full items-center gap-2 rounded-lg border border-slate-800 bg-slate-950/30 px-3 py-2 text-left text-xs text-slate-300 hover:border-cyan-500/30 hover:bg-cyan-500/[0.06]"
              >
        ...
        <div className="mt-auto rounded-xl border border-slate-800 bg-slate-950/35 p-3 text-xs leading-relaxed text-slate-500">
  ...
  return (
    <>
      <div className="border-b border-slate-800 bg-[#0d1420] p-4">
        <div className="mb-1 text-[11px] text-slate-500">Knowledge / <b className="text-cyan-300">{f.title}</b></div>
        {focusFresh && (
          <div className="mb-3 inline-flex items-center gap-1.5 rounded-full border border-slate-700 bg-slate-950/40 px-2.5 py-1 font-mono text-[11px] text-slate-300">
      ...
      <div className="space-y-3 p-4">
        ...
        {pack?.notes.length === 0 && !creating ? (
          <div className="rounded-xl border border-dashed border-slate-700 p-4 text-center text-xs leading-relaxed text-slate-500">
            还没有关联笔记。点 <span className="text-cyan-300">记录</span> 写下为什么、事故结论或决策，后续搜索和 AI 都能复用。
          </div>
        ) : pack?.notes.map((n) => (
          <button key={n.nodeId} type="button" onClick={() => onEditNote(n.title)} className="flex w-full items-center gap-2 rounded-lg border border-slate-800 bg-slate-950/30 px-3 py-2 text-left text-xs hover:border-cyan-500/30 hover:bg-cyan-500/[0.06]">
