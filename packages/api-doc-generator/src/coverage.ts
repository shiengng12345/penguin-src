import type { CoverageSummary, DocumentationRevision, EndpointDoc, EvidenceGap, RequestClassAnalysis, ResponseClassAnalysis } from "./types.js";
import type { EvidenceValidation } from "./evidence.js";
export function deriveCoverage(input: { endpointKey: string; schemaGaps: EvidenceGap[]; requestAnalysis: RequestClassAnalysis; responseAnalysis: ResponseClassAnalysis; revisionTrust: DocumentationRevision["trust"]; evidenceValidation: EvidenceValidation; testCoveredClassIds: string[]; runtimeObservedClassIds: string[]; runtimeEvidenceState: CoverageSummary["runtimeEvidenceState"] }): CoverageSummary {
  const blockers = [...input.schemaGaps, ...input.evidenceValidation.gaps, ...input.requestAnalysis.unresolvedConstraintIds.map((id) => ({ gapId: `gap_constraint_${id}`, code: "unresolved_constraint", message: `Constraint ${id} was not converted to a bounded partition.`, endpointKey: input.endpointKey, evidenceIds: [] })), ...input.responseAnalysis.unresolvedProducerIds.map((id) => ({ gapId: `gap_producer_${id}`, code: "unresolved_producer", message: `Response producer ${id} remains unresolved.`, endpointKey: input.endpointKey, producerId: id, evidenceIds: [] }))];
  if (input.revisionTrust === "fallback_live" || input.revisionTrust === "trust_unavailable") blockers.push({ gapId: `gap_trust_${input.endpointKey}`, code: "revision_trust_degraded", message: `Revision trust is ${input.revisionTrust}.`, endpointKey: input.endpointKey, evidenceIds: [] });
  if (input.runtimeEvidenceState === "unavailable" || input.runtimeEvidenceState === "partial") blockers.push({ gapId: `gap_runtime_${input.endpointKey}`, code: "runtime_evidence_unavailable", message: "Requested runtime evidence is unavailable or partial.", endpointKey: input.endpointKey, evidenceIds: [] });
  const dynamic = input.responseAnalysis.dynamicProducerIds.length;
  const level: CoverageSummary["level"] = blockers.length ? "partial" : dynamic ? "bounded" : "exhaustive";
