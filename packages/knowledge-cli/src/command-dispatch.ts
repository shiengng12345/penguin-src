...
export async function dispatchCliCommand(argv: string[], deps: CliDeps, parsed = parseCliArguments(argv)): Promise<number> {
  ...
  if (verb === "source") {
    ...
    try {
      ...
      if (pos[0] === "register") { const type = optionValue("type") ?? pos[1]; const location = optionValue("location") ?? pos[2]; if (!type || !location) { deps.err("usage: penguin source register --type <markdown_directory|url|postgres_schema|openapi> --location <value>"); return 2; } const allowHosts = (optionValue("allow-hosts") ?? "").split(",").map((host) => host.trim()).filter(Boolean); const credentialEntryId = optionValue("credential-id"); const schemas = (optionValue("schema") ?? "").split(",").map((schema) => schema.trim()).filter(Boolean); const result = sources.register({ type: type as import("@penguin/knowledge-core").ExternalKnowledgeSourceType, location, config: { ...(credentialEntryId ? { credentialEntryId } : {}), ...(schemas.length ? { schemas } : {}) }, ...(allowHosts.length ? { allowHosts } : {}) }); emit(deps, json, `registered ${result.id}`, result); return 0; }
  ...
  if (verb === "saved-query") {
    ...
    try {
      ...
      if (action === "write") {
        ...
        emit(deps, json, `saved ${result.name}`, { ...result, ...(markdownPath ? { markdownPath } : {}) }); return 0;
      }
      if (action === "run") {
  ...
  if (verb === "init" || verb === "index" || verb === "rebuild") {
    ...
    let rootPath: string;
    try { rootPath = roots ? assertWorkspacePath(pos[0] ?? deps.cwd, roots, "index root") : canonicalPathForCheck(pos[0] ?? deps.cwd); }
    catch (error) { deps.err((error as Error).message); return 2; }
    ...
    if (!isGitRepo(rootPath)) {
      ...
      if (subs.length > 0) {
        if (deps.pickRepos) {
          ...
          try { targets = picked.map((target) => roots ? assertWorkspacePath(target, roots, "selected index root") : canonicalPathForCheck(target)); }
          catch (error) { deps.err((error as Error).message); return 2; }
        } else {
          deps.err(
  ...
  if (verb === "watch") {
    ...
    const roots = configuredRoots === undefined ? null : parseWorkspaceRoots(configuredRoots, deps.cwd);
    let rootPath: string;
    try { rootPath = roots ? assertWorkspacePath(pos[0] ?? deps.cwd, roots, "watch root") : canonicalPathForCheck(pos[0] ?? deps.cwd); }
    catch (error) { deps.err((error as Error).message); return 2; }
    const store = deps.openStore();
    const emitEvents = flags.includes("--progress-events") || flags.includes("--events-jsonl");
