...
const ALL_STAGE_IDS = ["scan", "parse", "deletes", "proto", "link", "packages", "git"];
const SPINNER = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
export function initialRenderState(label, mode, now) {
    const stages = Object.fromEntries(ALL_STAGE_IDS.map((id) => [id, { state: "pending" }]));
    return {
...
export function applyEvent(state, ev, color, now) {
    switch (ev.phase) {
        case "scan":
            state.total = ev.total;
            if (ev.langs)
                state.langTotals = ev.langs;
            return null;
        case "index":
            state.done = ev.done;
            state.total = ev.total;
            state.file = ev.file;
            if (ev.lang)
                state.langDone[ev.lang] = (state.langDone[ev.lang] ?? 0) + 1;
            return null;
        case "stage": {
...
const KIND_LABEL = { endpoint: "endpoint", service: "service", link: "link" };
export function formatDiscoveryLine(ev, color) {
...
function bar(done, total, width, c, color = "36") {
...
const LANG_FALLBACK = ["38;2;143;143;143", "38;5;245"]; // neutral gray: Other + unknown
function langColor(label) {
...
export function langGroups(state) {
...
export function renderRegionLines(state, width, color, now) {
    const c = painter(color);
    const lines = [];
    ...
    for (const { id, label } of STAGES) {
        ...
        if (s.state === "done") {
            ...
            if (id === "parse" && state.total > 0) {
                ...
                if (state.file)
                    lines.push(`      ${c("2", `└─ ${truncPath(state.file, Math.max(20, width - 10))}`)}`);
            }
            else {
    ...
    lines.push(`  ${c("2", "Symbols")} ${fmtInt(state.symbols)}   ${c("2", "Edges")} ${fmtInt(state.edges)}   ${c("2", "Endpoints")} ${fmtInt(state.endpoints)}`);
    return lines;
}
...
function etaText(state, now) {
    if (!state.parseStartedAt || state.done < 10 || state.total === 0)
        return null;
...
export function summaryLines(report, state, elapsedMs, color) {
    const c = painter(color);
    ...
    const stat = (n, label) => `    ${c("1", fmtInt(n))} ${label}`;
    const lines = [];
    ...
    if (state.endpoints > 0)
        lines.push(stat(state.endpoints, "endpoints"));
    if (report.skipped > 0)
        lines.push(`    ${c("2", `${fmtInt(report.skipped)} unchanged`)}`);
    if (report.errors > 0) {
...
export function createIndexRenderer(opts) {
    ...
    const erase = () => {
        if (regionLines > 0)
            opts.write(`\x1b[${regionLines}F\x1b[J`);
        regionLines = 0;
    };
    const draw = () => {
    ...
    return {
        handle(ev) {
            ...
            if (!structural && t - lastDraw < 80)
                return;
            lastDraw = t;
            erase();
            if (discovery)
                opts.write(discovery + "\n");
            draw();
        },
        finish(report) {
            finished = true;
...
//# sourceMappingURL=render-progress.js.map