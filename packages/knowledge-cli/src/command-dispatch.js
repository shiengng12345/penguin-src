...
import { buildContextPack, buildExplorePack, renderContextPackMarkdown, buildFlow, renderFlowMarkdown, affectedByFiles, architecture, communities, timeline, endpointSamples, resolveEndpointId, deadCode, compareBranches, exploreGraph, getNodeDetail, graphNeighborhood, serviceGraph, indexStatus, compactIndexStatus, listFileSymbols, listIndexedFiles, listSuggestions, listTags, repoGraph, planRevisionCollection, applyRevisionCollection, search, searchSource, searchPath, getSourceHit, searchRegex, searchKnowledge, graphQuery, packageDependencies, dependencyPath, OntologyStore, buildDomainClaims, buildDomainFlow, filterHitsByPropertyPredicates, filterHitsByMarkdownPredicates, buildOnboardingDocument, WhyCardStore, MemoryStore, exportKnowledgeArtifact, previewKnowledgeArtifact, importKnowledgeArtifact, inspectKnowledgeArtifact, restoreKnowledgeArtifact, backfillSourceCorpus, SavedQueryStore, writeSavedQueryMarkdown, reflectSearchFeedback, ExternalSourceStore, fingerprintMarkdownDirectory, syncRemoteSource, syncPostgresSchema, resolveSymbolMatches, renderAmbiguousSymbols, requireRevisionContext, parseWorkspaceRoots, assertWorkspacePath, canonicalPathForCheck, RevisionResolutionError, compileKnowledgeDsl, resolveQueryScope, resolveRepoForPath, ScopeResolutionError, } from "@penguin/knowledge-core";
import { indexRepo, indexRevision, RevisionIndexCoordinator, KNOWLEDGE_PARSER_VERSION, KNOWLEDGE_RESOLVER_VERSION, startWatcher, createNote, createIncident, appendNote, writeNoteBody, readNote, listNotes, reindexNotesDir, listDanglingNoteLinks, listEvidenceNotes, setEvidenceStatus, evidenceDoctor, repairEvidence, readGitContext } from "@penguin/knowledge-indexer";
...
import { discoverSubRepos, isGitRepo } from "./multi-repo.js";
import { runApiDocCommand } from "./api-doc-command.js";
import { createKnowledgeApiDocAdapter } from "./api-doc-knowledge-adapter.js";
import { LarkCliDocumentClient } from "./lark-document-client.js";
import { CAPABILITIES, capabilityHash, listCliRegistrations, warning } from "@penguin/knowledge-contracts";
...
function resolveRepoId(store, s) {
    if (!s)
        return null;
    const row = store.db
...
function resolveBranchId(store, repoId, s) {
    if (!s) {
...
function reportRevisionResolutionError(deps, error) {
    if (!(error instanceof RevisionResolutionError))
        throw error;
    const candidates = error.candidates.slice(0, 20).map((candidate) => `${candidate.branch ?? "(unnamed)"} commit=${candidate.commitSha} --branch ${candidate.branch ?? ""}`);
    deps.err([
        error.message,
...
function reportScopeResolutionError(deps, error, json) {
...
function resolveRepoIdFromSnapshot(store, snapshotId) {
    if (snapshotId.startsWith("legacy:")) {
        ...
        const row = store.db.prepare("SELECT repo_id AS repoId FROM branches WHERE id=? AND status <> 'gone'").get(branchId);
        return row?.repoId;
    }
    const row = store.db.prepare("SELECT repo_id AS repoId FROM revision_snapshots WHERE id=?").get(snapshotId);
    return row?.repoId;
}
function resolveCliRevision(store, target, selector, cwd, options) {
        ...
        if (inferredFromQuery) {
            return {
                ...
                scope: {
                    ...
                    warnings: [
                        ...scope.warnings,
                        warning("REPO_INFERRED_FROM_QUERY", `search scope inferred from query text match in ${scope.locator.repoName}, not your working directory`),
                    ],
                },
            };
        }
...
function fallbackWarningFor(data, scope) {
    if (!data || typeof data !== "object" || Array.isArray(data))
        return undefined;
    const scopeFallback = data.scopeFallback;
    if (!scopeFallback?.branchId)
        return undefined;
    if (scope?.alignment === "fallback")
        return undefined; // already flagged via the scope envelope
    return warning("FALLBACK_LIVE_BRANCH", `no revision was resolved for this query; answered against the live branch (${scopeFallback.branchId}) instead of an explicit revision`, { branchId: scopeFallback.branchId });
}
function emit(deps, json, human, data, scope) {
...
function emitProgress(deps, payload) {
    if (EVENT_OUTPUT.get(deps))
        deps.out(JSON.stringify({ type: "progress", payload }));
    deps.progressEvent?.(payload);
}
function operationToken(operation, scope) {
    return createHash("sha256").update(JSON.stringify({ operation, scope })).digest("hex").slice(0, 32);
}
function confirmationValue(argv) {
...
function confirmationAccepted(argv, expected) {
    const value = confirmationValue(argv);
    return value === expected || value === "legacy-confirm";
}
function requireOperationToken(deps, argv, operation, scope) {
    if (!deps.requireOperationConfirmation)
        return true;
    if (process.env.PENGUIN_KNOWLEDGE_TRUSTED_BACKGROUND === "1" && ["init", "index", "rebuild"].includes(operation))
        return true;
...
export async function dispatchCliCommand(argv, deps, parsed = parseCliArguments(argv)) {
    ...
    const artifactPassphrase = () => {
        const envName = optionValue("passphrase-env");
        if (envName) {
            const value = process.env[envName];
            if (!value)
                throw new Error("ARTIFACT_PASSPHRASE_ENV_EMPTY");
            return value;
        }
        ...
        if (fdValue !== undefined) {
            ...
            if (!Number.isInteger(fd) || fd < 0)
                throw new Error("ARTIFACT_PASSPHRASE_FD_INVALID");
            const value = readFileSync(fd, "utf8").replace(/[\r\n]+$/u, "");
            if (!value)
                throw new Error("ARTIFACT_PASSPHRASE_FD_EMPTY");
            return value;
        }
        return undefined;
    };
    if (verb === "api-doc") {
        ...
        try {
            return await runApiDocCommand(argv.slice(1), { cwd: deps.cwd, out: deps.out, err: deps.err, json, previewRoot: deps.apiDocPreviewRoot ?? `${deps.cwd}/.penguin/api-docs/previews`, sourceAdapter: deps.apiDocSourceAdapter ?? (apiStore ? createKnowledgeApiDocAdapter(apiStore) : undefined), readStdin: deps.readStdin, bindingPath: deps.apiDocBindingPath, larkClient: deps.apiDocLarkClient ?? (deps.larkProcessRunner ? new LarkCliDocumentClient(deps.larkProcessRunner) : undefined) });
        }
        finally {
            apiStore?.close();
        }
    ...
    if (verb === "artifact") {
        if (!deps.storeExists()) {
            deps.err("no knowledge database — run `penguin init` first");
            return 3;
        }
        const action = pos[0] ?? "";
        ...
        try {
            if (action === "export") {
                let encryptionKey;
                try {
                    encryptionKey = artifactPassphrase();
                }
                catch (error) {
                ...
                if (repoOption && !scopedRepoId) {
                    deps.err(`unknown repo: ${repoOption}`);
                    return 2;
                }
                if (snapshotOption && !(store.db.prepare("SELECT 1 FROM revision_snapshots WHERE id=?").get(snapshotOption))) {
                    deps.err(`unknown snapshot: ${snapshotOption}`);
                    return 2;
                }
                ...
                if (!confirmationAccepted(argv, token)) {
                    emit(deps, json, "artifact export requires confirmation; rerun with --confirm=<operation-token> after reviewing this preview", { ...previewKnowledgeArtifact(store, previewOptions), operationToken: token });
                    return 6;
                }
                if (confirmationValue(argv) && confirmationValue(argv) !== token && confirmationValue(argv) !== "legacy-confirm") {
                    deps.err("confirmation token does not match the requested artifact export scope");
                    return 6;
                }
                ...
                if (!output) {
                    deps.err("usage: penguin artifact export --out <artifact.pka>");
                    return 2;
                }
            ...
            if (action === "import") {
                ...
                if (!input) {
                    deps.err("usage: penguin artifact import --input <artifact.pka> [--capability-hash <hash>]");
                    return 2;
                }
                ...
                if (destination) {
                    ...
                    if (!confirmationAccepted(argv, token)) {
                        deps.err(`artifact restore is guarded; repeat with --confirm=<operation-token> (token: ${token})`);
                        return 2;
                    }
                    if (confirmationValue(argv) !== token && confirmationValue(argv) !== "legacy-confirm") {
                        deps.err("confirmation token does not match the requested artifact restore scope");
                        return 2;
                    }
                    const basePath = optionValue("base");
            ...
            deps.err("usage: penguin artifact export|import");
            return 2;
        }
        finally {
            store.close();
    ...
    if (verb === "source") {
        ...
        try {
            ...
            if (pos[0] === "list") {
                const result = sources.list();
                emit(deps, json, result.map((source) => `${source.id}\t${source.type}\t${source.location}`).join("\n") || "(no external sources)", result);
                return 0;
            }
            if (pos[0] === "register") {
                const type = optionValue("type") ?? pos[1];
                const location = optionValue("location") ?? pos[2];
                if (!type || !location) {
                    deps.err("usage: penguin source register --type <markdown_directory|url|postgres_schema|openapi> --location <value>");
                    return 2;
                }
                const allowHosts = (optionValue("allow-hosts") ?? "").split(",").map((host) => host.trim()).filter(Boolean);
                const credentialEntryId = optionValue("credential-id");
                const schemas = (optionValue("schema") ?? "").split(",").map((schema) => schema.trim()).filter(Boolean);
                const result = sources.register({ type: type, location, config: { ...(credentialEntryId ? { credentialEntryId } : {}), ...(schemas.length ? { schemas } : {}) }, ...(allowHosts.length ? { allowHosts } : {}) });
                emit(deps, json, `registered ${result.id}`, result);
                return 0;
            }
            if (pos[0] === "remove") {
                const id = optionValue("id") ?? pos[1];
                if (!id || !flags.includes("--confirm")) {
                    deps.err("usage: penguin source remove <id> --confirm");
                    return 2;
                }
                sources.remove(id);
                emit(deps, json, `removed ${id}`, { ok: true, id });
                return 0;
            }
            if (pos[0] === "sync") {
                const id = optionValue("id") ?? pos[1];
                ...
                if (!source) {
                    emit(deps, json, "external source not found", { error: "EXTERNAL_SOURCE_NOT_FOUND" });
                    return 2;
                }
                ...
                if (source.type === "postgres_schema") {
                    const result = await syncPostgresSchema(store, source.id, deps.postgresSchemaClient);
            ...
            if (pos[0] === "backfill") {
                ...
                if (selector && !repoId) {
                    emit(deps, json, "repository not found", { error: "REPOSITORY_NOT_FOUND", selector });
                    return 2;
                }
            ...
            deps.err("usage: penguin source register|list|sync|backfill|remove");
            return 2;
        }
        finally {
            store.close();
    ...
    if (verb === "saved-query") {
        ...
        try {
            ...
            if (action === "list") {
                const result = saved.list(optionValue("query"));
                emit(deps, json, result.map((item) => `${item.name}\t${item.updatedAt}`).join("\n") || "(no saved queries)", result);
                return 0;
            }
            if (action === "write") {
                ...
                if (!name || !raw) {
                    deps.err("usage: penguin saved-query write <name> <request-json>");
                    return 2;
                }
                let request;
                try {
                    ...
                    deps.err("saved query request must be valid JSON");
                    return 2;
                }
                const result = saved.write({ name, request, scope: {} });
                const markdownPath = deps.notesDir ? writeSavedQueryMarkdown(deps.notesDir, result) : undefined;
                emit(deps, json, `saved ${result.name}`, { ...result, ...(markdownPath ? { markdownPath } : {}) });
                return 0;
            }
            if (action === "run") {
                const result = saved.get(pos[1] ?? optionValue("name") ?? "");
                if (!result) {
                    deps.err("saved query not found");
                    return 1;
                }
                const response = await searchKnowledge(result.request, { store });
                emit(deps, json, JSON.stringify(response), response);
                return 0;
            }
            deps.err("usage: penguin saved-query list|write|run");
            return 2;
        }
        finally {
            store.close();
    ...
    if (verb === "memory" && pos[0] === "improve") {
        ...
        if (!flags.includes("--confirm")) {
            deps.err("memory improve is guarded; repeat with --confirm");
            return 6;
        }
        const store = deps.openStore();
        try {
    ...
    if (verb === "package-dependencies" || verb === "dependency-path") {
        ...
        try {
            if (verb === "package-dependencies") {
                ...
                if (!subject) {
                    deps.err("usage: penguin package-dependencies <package> [--direction dependencies|dependents|both]");
                    return 2;
                }
                const result = packageDependencies(store, { subject, direction: optionValue("direction") ?? "dependencies", transitive: !flags.includes("--direct"), maxDepth: numberOption("depth") ?? 5, limit: numberOption("limit") ?? 100 });
                emit(deps, json, JSON.stringify(result), result);
                return 0;
            }
            const from = pos[0], to = pos[1];
            if (!from || !to) {
                deps.err("usage: penguin dependency-path <from> <to>");
                return 2;
            }
    ...
    if (verb === "ontology") {
        ...
        try {
            ...
            if (pos[0] === "list" || !pos[0]) {
                const result = ontology.list();
                emit(deps, json, result.map((term) => `${term.id}\t${term.canonicalName}\t${term.status}`).join("\n") || "(no ontology terms)", result);
                return 0;
            }
            if (pos[0] === "upsert") {
                if (!flags.includes("--confirm")) {
                    deps.err("ontology upsert is guarded; repeat with --confirm");
                    return 6;
                }
                const term = JSON.parse(pos.slice(1).join(" "));
                const result = ontology.upsert(term);
                if (result.status === "ambiguous") {
                    emit(deps, json, "ontology alias conflicts with existing candidates", { ok: false, code: "ONTOLOGY_ALIAS_AMBIGUOUS", candidates: result.candidates });
                    return 6;
                }
                emit(deps, json, `upserted ${term.id}`, { ok: true, id: term.id, resolution: result });
                return 0;
            }
            if (pos[0] === "link") {
                if (!flags.includes("--confirm")) {
                    deps.err("ontology link is guarded; repeat with --confirm");
                    return 6;
                }
                ontology.link(pos[1] ?? "", pos[2] ?? "", pos[3] ?? "related_to");
                emit(deps, json, "linked ontology terms", { ok: true });
                return 0;
            }
            deps.err("usage: penguin ontology list|upsert|link");
            return 2;
        }
        finally {
            store.close();
    ...
    if (verb === "analyze-repository") {
        ...
        try {
            ...
            if (!queryText) {
                deps.err("usage: penguin analyze-repository <question>");
                return 2;
            }
    ...
    if (verb === "hook") {
        ...
        const store = deps.openStore();
        try {
            const output = await runClaudeHook({ event, prompt }, {
    ...
    if (verb === "init" || verb === "index" || verb === "rebuild") {
        ...
        let rootPath;
        try {
            rootPath = roots ? assertWorkspacePath(pos[0] ?? deps.cwd, roots, "index root") : canonicalPathForCheck(pos[0] ?? deps.cwd);
        }
        catch (error) {
        ...
        if (!isGitRepo(rootPath)) {
            ...
            if (subs.length > 0) {
                if (deps.pickRepos) {
                    ...
                    if (picked == null)
                        return 0; // cancelled — nothing indexed
                    if (picked.length === 0) {
                        deps.err("nothing selected");
                        return 0;
                    }
                    try {
                        targets = picked.map((target) => roots ? assertWorkspacePath(target, roots, "selected index root") : canonicalPathForCheck(target));
                    }
                    catch (error) {
                        deps.err(error.message);
                        return 2;
                    ...
                    deps.err(`${rootPath} is not a git repo but contains ${subs.length} git repos — pass one explicitly:`);
                    for (const s of subs)
                        deps.err(`  penguin ${verb} ${s.path}`);
                    return 2;
                }
            }
        }
        ...
        if (!requireOperationToken(deps, argv, verb, { targets }))
            return 6;
        const store = deps.openStore();
        try {
            ...
            for (const target of targets) {
                ...
                const report = await indexRepo({
                    ...
                    onProgress: emitEvents
                        ? (p) => {
                            ...
                            if (deps.progressEvent)
                                deps.progressEvent({ ...p, rootPath: target });
                        }
                        : renderer
                            ? (p) => renderer.handle(p)
                            : undefined,
                ...
                if (!renderer) {
                    emit(deps, json, `${verb}: ${report.branchName} — ${report.parsed} parsed, ${report.skipped} skipped, ${report.deleted} deleted, ${report.renamed} renamed, ${report.errors} errors`, report);
                }
            }
            return 0;
        }
    ...
    if (verb === "watch") {
        ...
        let rootPath;
        try {
            rootPath = roots ? assertWorkspacePath(pos[0] ?? deps.cwd, roots, "watch root") : canonicalPathForCheck(pos[0] ?? deps.cwd);
        }
        catch (error) {
            deps.err(error.message);
            return 2;
        ...
        const handle = startWatcher({
            ...
            onRun: (report) => {
                if (emitEvents)
                    emitProgress(deps, { phase: "watch-run", rootPath, report });
                else
                    deps.out(`watch: ${report.branchName} — ${report.parsed} parsed, ${report.errors} errors`);
            },
        });
        ...
        if (emitEvents)
            emitProgress(deps, { phase: "watch-started", rootPath });
    ...
    if (verb === "install") {
        if (deps.installSelf) {
            try {
                ...
                deps.err(`install failed: ${e.message}`);
                return 1;
            }
        }
    ...
    if (verb === "remove" || verb === "pin") {
        const target = pos[0];
        if (!target) {
            deps.err(`usage: penguin ${verb} <repo-name|path> [branch]`);
            return 2;
        }
        if (!deps.storeExists()) {
            deps.err("no knowledge database");
            return 3;
        }
        const store = deps.openStore();
        try {
            ...
            if (!requireOperationToken(deps, argv, verb, scope))
                return 6;
            if (verb === "pin") {
                if (!branchName) {
                    deps.err("usage: penguin pin <repo> <branch>");
                    return 2;
                }
                const br = store.getBranch(row.id, branchName);
                if (!br) {
                    deps.err(`no branch "${branchName}" in ${row.name}`);
                    return 1;
                }
    ...
    if (verb === "note") {
        ...
        if (!notesDir) {
            deps.err("notes dir not configured");
            return 1;
        }
        const store = deps.openStore();
        try {
            if (sub === "new") {
                ...
                if (!title) {
                    deps.err("usage: penguin note new <title> [--type=decision|incident|compliance|bug|requirement|architecture]");
                    return 2;
                }
            ...
            if (sub === "append") {
                ...
                if (!slug || !text) {
                    deps.err("usage: penguin note append <slug> <text>");
                    return 2;
                }
                try {
            ...
            if (sub === "write") {
                ...
                const body = pos[2] ?? "";
                if (!slug) {
                    deps.err("usage: penguin note write <slug> <body>");
                    return 2;
                }
                try {
            ...
            if (sub === "read") {
                const slug = pos[1];
                if (!slug) {
                    deps.err("usage: penguin note read <slug>");
                    return 2;
                }
                const src = readNote(notesDir, slug);
                if (src == null) {
                    deps.err(`note not found: ${slug}`);
                    return 1;
                }
    ...
    if (verb === "incident") {
        ...
        const title = pos.slice(1).join(" ").trim();
        if (sub !== "new" || !title) {
            deps.err("usage: penguin incident new <title>");
            return 2;
        }
        const store = deps.openStore();
        try {
    ...
    if (verb === "evidence") {
        ...
        try {
            if (sub === "plan") {
                ...
                if (!question) {
                    deps.err("usage: penguin evidence plan <question> [--target <target-id>]");
                    return 2;
                }
                ...
                emit(deps, json, `planned ${planId}`, plan);
                return 0;
            }
            if (sub === "capture") {
                const planId = pos[1];
                const rawResults = optionValue("results");
                if (!planId || !rawResults) {
                    deps.err("usage: penguin evidence capture <plan-id> --results <json>");
                    return 2;
                }
                let results;
                try {
                    results = JSON.parse(readFileSync(rawResults, "utf8"));
                }
                catch {
                    try {
                        ...
                        deps.err("results must be a JSON file or JSON value");
                        return 2;
                    }
                }
                const targetId = optionValue("target") ?? "knowledge-local";
                ...
                emit(deps, json, `captured ${note.slug}`, { ok: true, planId, targetId, note });
                return 0;
            }
            if (sub === "list") {
                const rows = listEvidenceNotes({ store, notesDir, targetId: optionValue("target"), status: optionValue("status"), limit: numberOption("limit") });
            ...
            if (sub === "get") {
                const slug = pos[1];
                if (!slug) {
                    deps.err("usage: penguin evidence get <slug>");
                    return 2;
                }
                ...
                if (!row || source == null) {
                    deps.err("evidence note not found");
                    return 1;
                }
                emit(deps, json, source, { ...row, markdown: source });
                return 0;
            }
            if (sub === "validate") {
            ...
            if (sub === "status") {
                const slug = pos[1];
                const to = pos[2];
                if (!slug || !to || !["reviewed", "verified", "resolved", "archived"].includes(to)) {
                    deps.err("usage: penguin evidence status <slug> <reviewed|verified|resolved|archived>");
                    return 2;
                }
                const row = setEvidenceStatus({ store, notesDir, slug, to, from: optionValue("from") });
    ...
    if (verb === "link" && (pos[0] === "list" || pos[0] === "delete")) {
        ...
        try {
            if (pos[0] === "list") {
                ...
                emit(deps, json, rows.map((row) => `${row.id}\t${row.src} → ${row.dst ?? "?"}\t${row.edgeType}`).join("\n") || "(no active links)", rows);
                return 0;
            }
            const id = pos[1];
            if (!id) {
                deps.err("usage: penguin link delete <edge-id> --confirm");
                return 2;
            }
            if (!flags.includes("--confirm")) {
                deps.err("link delete is guarded; repeat with --confirm");
                return 6;
            }
            ...
            emit(deps, json, `deleted link ${id}`, { ok: true, eventId: event.id, edgeId: id });
            return 0;
        }
        finally {
            store.close();
    ...
    if (verb === "revisions") {
        ...
        if (sub === "migrate") {
            ...
            try {
                ...
                const repos = store.db.prepare(requested ? "SELECT id,name,root_path AS rootPath FROM repos WHERE id=? OR name=?" : "SELECT id,name,root_path AS rootPath FROM repos").all(...(requested ? [requested, requested] : []));
                const integrity = (repoId) => {
                    const count = (sql, ...args) => Number(store.db.prepare(sql).get(...args)?.n ?? 0);
                    return {
                ...
                const report = repos.map((repo) => {
                    const masterCount = store.db.prepare("SELECT COUNT(*) AS n FROM branches WHERE repo_id=? AND default_branch=1").get(repo.id).n;
                    return {
                        repoId: repo.id,
                        repo: repo.name,
                        rootPath: repo.rootPath,
                        legacyFiles: store.db.prepare("SELECT COUNT(*) AS n FROM files_index WHERE repo_id=?").get(repo.id).n,
                        notes: store.db.prepare("SELECT COUNT(*) AS n FROM notes_index ni JOIN nodes n ON n.id=ni.node_id WHERE n.repo_id=?").get(repo.id).n,
                ...
                if (!flags.includes("--apply")) {
                    emit(deps, json, `migration dry-run: ${report.length} repo(s)`, { mode: "dry-run", repositories: report, notesAndLedgerPreserved: true, legacyRowsRetained: true });
                    return 0;
                }
                const results = [];
                let failed = false;
                for (const repo of repos) {
                    const pointers = store.db.prepare("SELECT id,current_snapshot_id,head_commit,last_indexed_commit,last_indexed_at FROM branches WHERE repo_id=?").all(repo.id);
                    try {
                        const rebuilt = await indexRepo({ store, rootPath: repo.rootPath, mode: "rebuild" });
                        const checks = integrity(repo.id);
                        const orphanCount = Object.values(checks).reduce((sum, value) => sum + value, 0);
                        if (orphanCount)
                            throw new Error(`integrity check failed: ${JSON.stringify(checks)}`);
                        results.push({ repo: repo.name, report: rebuilt, integrity: checks, status: "migrated" });
                    }
                    catch (error) {
                        failed = true;
                        const restore = store.db.transaction(() => { for (const pointer of pointers)
                            store.db.prepare("UPDATE branches SET current_snapshot_id=?,head_commit=?,last_indexed_commit=?,last_indexed_at=? WHERE id=?").run(pointer.current_snapshot_id, pointer.head_commit, pointer.last_indexed_commit, pointer.last_indexed_at, pointer.id); });
                        restore();
                        results.push({ repo: repo.name, status: "rolled_back", error: String(error.message ?? error), integrity: integrity(repo.id) });
                    }
                }
        ...
        if (sub !== "gc") {
            deps.err("usage: penguin revisions gc <repo> [--dry-run|--apply] | migrate [--dry-run|--apply]");
            return 2;
        }
        const repoName = pos[1] ?? optionValue("repo");
        if (!repoName || !deps.storeExists()) {
            deps.err("usage: penguin revisions gc <repo> [--dry-run|--apply]");
            return 2;
        }
        const store = deps.openStore();
        try {
            ...
            if (!repoId) {
                deps.err(`repo not found: ${repoName}`);
                return 1;
            }
    ...
    if (verb === "materialize") {
        ...
        try {
            ...
            const repo = store.db.prepare("SELECT root_path AS rootPath FROM repos WHERE id=?").get(repoId);
            ...
            deps.err(`materialize failed: ${String(error.message ?? error)}`);
            return 1;
        }
        finally {
            store.close();
    ...
    if (verb === "explain") {
        ...
        if (!target) {
            deps.err("usage: penguin explain <symbol|endpoint> [--provider=deepseek|openai] [--model=…] [--key=…]");
            return 2;
        }
        const flagVal = (name) => flags.find((fl) => fl.startsWith(`--${name}=`))?.slice(name.length + 3);
        const store = deps.openStore();
        try {
            const pack = buildContextPack(store, target);
            if (!pack.focus) {
                if (pack.ambiguous)
                    deps.err(renderAmbiguousSymbols(target, pack.ambiguous));
                else if (pack.assemblyError)
                    deps.err(`"${target}" resolved to a symbol, but building its context failed: ${pack.assemblyError}`);
                else
                    deps.err(`no context for "${target}" — not indexed, or the name doesn't match any symbol/note`);
                return 1;
            }
    ...
    if (verb === "sample") {
        ...
        if (!endpoint || !payload) {
            deps.err("usage: penguin sample <endpoint> <status> <json-or-text> [contentType]");
            return 2;
        }
        const store = deps.openStore();
        try {
    ...
    if (verb === "accept" || verb === "reject" || verb === "link" || verb === "snapshot" || verb === "remember" || verb === "forget") {
        ...
        try {
            if (verb === "link") {
                ...
                if (!src || !dst) {
                    deps.err("usage: penguin link <src> <dst> [edgeType]");
                    return 2;
                }
                const ev = store.recordKnowledge({
            ...
            if (verb === "snapshot") {
                ...
                if (!name) {
                    deps.err("usage: penguin snapshot <name> <nodeId...>");
                    return 2;
                }
            ...
            if (verb === "remember") {
                ...
                if (!subject || !body) {
                    deps.err("usage: penguin remember <subject> <body> [--repo <repo>|--workspace <id>|--global] [--class=decision|runbook|incident|preference|project|session]");
                    return 2;
                }
                if (!optionValue("repo") && !optionValue("workspace") && !flags.includes("--global")) {
                    deps.err("remember requires an explicit scope: --repo, --workspace, or --global");
                    return 2;
                }
                const item = new MemoryStore(store).remember({
                    class: optionValue("class") ?? "project",
            ...
            if (verb === "forget") {
                const id = pos[0];
                if (!id) {
                    deps.err("usage: penguin forget <memory-id>");
                    return 2;
                }
                if (!flags.includes("--confirm")) {
                    deps.err("forget is guarded; repeat with --confirm after reviewing the memory id");
                    return 2;
                }
            ...
            const id = pos[0];
            if (!id) {
                deps.err(`usage: penguin ${verb} <suggestion-event-id>`);
                return 2;
            }
            if (verb === "accept")
            ...
            switch (verb) {
                case "coverage": {
                    ...
                    if (repoSelector && !repoId) {
                        deps.err(`unknown repo: ${repoSelector}`);
                        return 2;
                    }
                    const summary = (repoId ? store.db.prepare("SELECT COUNT(*) AS discovered, SUM(coverage_status='admitted') AS admitted, SUM(coverage_status<>'admitted') AS excluded, SUM(coverage_status='failed') AS failed FROM coverage_records WHERE repo_id=?").get(repoId) : store.db.prepare("SELECT COUNT(*) AS discovered, SUM(coverage_status='admitted') AS admitted, SUM(coverage_status<>'admitted') AS excluded, SUM(coverage_status='failed') AS failed FROM coverage_records").get());
                ...
                case "onboarding": {
                    ...
                    if (flags.includes("--save")) {
                        if (confirmationValue(argv) !== token) {
                            deps.err(`onboarding save is guarded; review the preview, then repeat with --confirm=${token}`);
                            return 6;
                        }
                        if (!deps.notesDir) {
                            deps.err("notes dir not configured");
                ...
                case "domain": {
                    ...
                    const result = { target: pos.join(" "), claims: buildDomainClaims(store, { ...(repoId ? { repoId } : {}), ...(optionValue("persona") ? { persona: optionValue("persona") } : {}) }), flow: buildDomainFlow(store, { ...(repoId ? { repoId } : {}), ...(pos.join(" ") ? { target: pos.join(" ") } : {}) }), gaps: repoId ? [] : ["repo scope not specified"] };
                    emit(deps, json, JSON.stringify(result), result);
                    return 0;
                }
                case "why": {
                ...
                case "search": {
                    ...
                    let dslPaths = [];
                    let propertyPredicates = [];
                    let markdownPredicates = [];
                    ...
                    if (revisionKinds.length > 1) {
                        deps.err("--snapshot, --commit, and --working-tree are mutually exclusive");
                        return 2;
                    }
                    ...
                    const selectedRepoIds = resolvedRepoIds.filter((repoId) => Boolean(repoId));
                    let revision;
                    let scope;
                    ...
                    const sourceSnapshotId = revision && !revision.snapshotId.startsWith("legacy:")
                        ? revision.snapshotId
                        : (revision?.branchId
                            ? store.db.prepare("SELECT current_snapshot_id FROM branches WHERE id=?").get(revision.branchId)?.current_snapshot_id ?? null
                            : null);
                    ...
                    if (useV2Search || optionValue("mode") || flags.includes("--compact") || optionValue("cursor")) {
                        const revisions = sourceSnapshotId
                            ...
                            : selectedRepoIds.length > 0
                                ...
                                : undefined;
                        let response = await searchKnowledge({ query: queryText, mode: mode, scope: { ...(revisions ? { revisions } : {}), ...(optionValue("workspace") ? { workspaceId: optionValue("workspace") } : {}), ...((optionValues("path").length || dslPaths.length) ? { paths: [...optionValues("path"), ...dslPaths] } : {}), ...(optionValues("language").length ? { languages: optionValues("language") } : {}), ...(optionValues("kind").length ? { kinds: optionValues("kind") } : {}) }, options: { caseSensitive: flags.includes("--case-sensitive") || !flags.includes("--case-insensitive"), wholeWord: flags.includes("--whole-word"), includeGenerated: flags.includes("--include-generated"), includeVendor: flags.includes("--include-vendor"), includeExcludedMetadata: flags.includes("--include-excluded-metadata"), semantic: optionValue("semantic") ?? "off", compact: flags.includes("--compact"), explain: flags.includes("--explain") }, page: { limit: numberOption("limit") ?? 50, ...(optionValue("cursor") ? { cursor: optionValue("cursor") } : {}) } }, { store, scopes: sourceSnapshotId ? [{ snapshotId: sourceSnapshotId, repoId }] : undefined });
                    ...
                    const regexResult = sourceSnapshotId && mode === "regex"
                        ...
                        : null;
                    if (regexResult?.status === "error") {
                        emit(deps, json, `${regexResult.code}: ${regexResult.message}`, regexResult, scope);
                        return 1;
                    }
                    const sourceHits = sourceSnapshotId && ["auto", "exact", "phrase", "substring"].includes(mode)
                        ? searchSource(store, { snapshotId: sourceSnapshotId, repoId }, { query: queryText, mode: mode, options: { caseSensitive: true, wholeWord: false, includeGenerated: true, includeVendor: true, includeExcludedMetadata: false, semantic: "off", compact: false, explain: false } }).map((hit) => ({ ...hit, lane: "source" }))
                        : [];
                ...
                case "graph-query": {
                    ...
                    if (!requestPath) {
                        deps.err("usage: penguin graph-query --request <json-file> --json");
                        return 2;
                    }
                    let request;
                    try {
                        ...
                        deps.err("invalid graph-query request JSON");
                        return 2;
                    }
                    try {
                        const result = graphQuery(store, request);
                        emit(deps, json, JSON.stringify(result), result);
                        return 0;
                    }
                    catch (error) {
                ...
                case "get-hit": {
                    ...
                    if (!snapshotId || !filePath) {
                        deps.err("usage: penguin get-hit <file-path> --snapshot <snapshot-id> [--line <n>|--start-byte <n>]");
                        return 2;
                    }
                ...
                case "context": {
                    ...
                    let scope;
                    try {
                        ({ revision, scope } = resolveCliRevision(store, target, { repo: optionValue("repo"), branch: optionValue("branch"), commitSha: optionValue("commit"), snapshotId: optionValue("snapshot"), allowFallback: flags.includes("--allow-fallback") }, deps.cwd));
                    }
                    catch (error) {
                        return reportScopeResolutionError(deps, error, json);
                    }
                ...
                case "explore": {
                    ...
                    let scope;
                    try {
                        ({ revision, scope } = resolveCliRevision(store, target, { repo: optionValue("repo"), branch: requestedBranch, commitSha: optionValue("commit"), snapshotId: optionValue("snapshot"), allowFallback: flags.includes("--allow-fallback") }, deps.cwd));
                    }
                    catch (error) {
                        return reportScopeResolutionError(deps, error, json);
                    }
                    let branchId;
                    if (requestedBranch) {
                        const exact = store.db.prepare("SELECT id FROM branches WHERE id=?").get(requestedBranch);
                        branchId = exact?.id;
                        if (!branchId) {
                            const resolution = resolveSymbolMatches(store, target);
                            const repoId = resolution.kind === "unique" ? store.getNode(resolution.nodeId)?.repo_id : null;
                            if (repoId)
                                branchId = resolveBranchId(store, repoId, requestedBranch) ?? undefined;
                        }
                        if (!branchId) {
                ...
                case "flow": {
                    ...
                    emit(deps, json, renderFlowMarkdown(flow), flow, scope);
                    if (flow.diagnostic)
                        return 1; // resolved, but a dead end — signal via exit code, still print the (partial) flow
                    return 0;
                }
                case "affected": {
                    ...
                    let scope;
                    try {
                        ({ revision, scope } = resolveCliRevision(store, "", { repo: optionValue("repo"), branch: optionValue("branch"), commitSha: optionValue("commit"), snapshotId: optionValue("snapshot"), allowFallback: flags.includes("--allow-fallback") }, deps.cwd));
                    }
                    catch (error) {
                        return reportScopeResolutionError(deps, error, json);
                    }
                ...
                case "compare": {
                    ...
                    let scope;
                    try {
                        ({ revision, scope } = resolveCliRevision(store, pos[0] ?? "", { repo: optionValue("repo"), branch: optionValue("branch"), commitSha: optionValue("commit"), snapshotId: optionValue("snapshot"), allowFallback: flags.includes("--allow-fallback") }, deps.cwd));
                    }
                    catch (error) {
                        return reportScopeResolutionError(deps, error, json);
                    }
                ...
                case "status": {
                    if (flags.includes("--revisions")) {
                        const rows = store.db.prepare(`SELECT r.name AS repo, b.name AS branch, b.status, b.current_snapshot_id AS snapshotId,
                 ...
                 LEFT JOIN revision_snapshots s ON s.id=b.current_snapshot_id ORDER BY r.name,b.name`).all();
                        const data = rows.map((row) => ({ ...row, baseCommit: row.baseSnapshotId ? store.db.prepare("SELECT commit_sha AS commitSha FROM revision_snapshots WHERE id=?").get(row.baseSnapshotId)?.commitSha ?? null : null, reusePercent: row.totalFiles ? 100 - (Number(row.changedFiles) / Number(row.totalFiles) * 100) : null, pinned: Boolean(store.db.prepare("SELECT pinned FROM branches WHERE name=? AND current_snapshot_id=?").get(row.branch, row.snapshotId)) }));
                    ...
                    if (compact) {
                        const summary = st;
                        emit(deps, json, summary.repos
                            .map((repo) => `${repo.repo}\t${repo.liveBranch ?? "—"}\t${repo.freshness}\terrors=${repo.indexErrorCount}`)
                            .join("\n") || "(no repos)", summary);
                        return 0;
                    }
                    const detailed = st;
                    emit(deps, json, detailed.repos.map((r) => `${r.name}\t${r.branches.map((b) => `${b.name}(${b.status},stale=${b.staleSymbols})`).join(" ")}`).join("\n") || "(no repos)", detailed);
                    return 0;
                }
                case "path": {
                    let revision;
                ...
                case "suggestions": {
                    ...
                    emit(deps, json, q.map((s) => `${s.suggestionEventId}\t${s.src} → ${s.dst ?? "?"}  (${s.edgeType}, conf ${s.confidence})`).join("\n") || "(no pending suggestions)", q);
                    return 0;
                }
                case "snapshots": {
                    const snaps = store.listSnapshots();
                    emit(deps, json, snaps.map((s) => `${s.ts}\t${s.name}\t${s.nodeIds.length} nodes`).join("\n") || "(no snapshots)", snaps);
                    return 0;
                }
                case "doctor": {
                    const check = store.consistencyCheck();
                    const nodes = store.db.prepare("SELECT COUNT(*) AS n FROM nodes").get().n;
                    const edges = store.db.prepare("SELECT COUNT(*) AS n FROM edges").get().n;
                    ...
                    emit(deps, json, `ledger seq ${check.ledgerSeq} / materialized ${check.materializedSeq} — ${check.status}` +
                        (check.ledgerTruncatedAtLine ? ` (ledger truncated @line ${check.ledgerTruncatedAtLine})` : "") +
                        `\nnodes ${nodes}, edges ${edges}, pending suggestions ${pending}`, report);
                    return 0;
                }
                case "files": {
                    const repoId = resolveRepoId(store, pos[0]);
                    if (!repoId) {
                        deps.err("repo not found (pass a repo id or name — see `penguin status`)");
                        return 1;
                    }
                    let branchId;
                    try {
                    ...
                    if (!branchId) {
                        deps.err("branch not found for that repo");
                        return 1;
                    }
                    let revision;
                    let scope;
                    try {
                        ({ revision, scope } = resolveCliRevision(store, pos[0] ?? "", { repo: pos[0], branch: optionValue("branch") ?? pos[1], commitSha: optionValue("commit"), snapshotId: optionValue("snapshot"), allowFallback: flags.includes("--allow-fallback") }, deps.cwd));
                    }
                    catch (error) {
                        return reportScopeResolutionError(deps, error, json);
                    }
                    const files = listIndexedFiles(store, repoId, revision ? { revision } : branchId);
                    emit(deps, json, files.map((f) => `${f.status === "indexed" ? " " : "·"} ${f.filePath}${f.lang ? `  [${f.lang}]` : ""}`).join("\n") || "(no files)", files, scope);
                    return 0;
                }
                case "filesymbols": {
