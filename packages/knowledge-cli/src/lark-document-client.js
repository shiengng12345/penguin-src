...
export function createLarkProcessRunner() {
    return {
        run(argv, stdin) {
...
function jsonEnvelope(result) {
    ...
    try {
        ...
        throw new Error(`lark-cli returned non-JSON output: ${result.stderr.slice(0, 1000)}`);
    }
    if (result.code === 10 || parsed.error?.type === "confirmation_required") {
        ...
        error.code = 10;
        error.detail = parsed.error ?? parsed;
        throw error;
    }
    if (result.code !== 0 || parsed.ok === false)
        throw new Error(parsed.error?.message ?? (result.stderr.slice(0, 1000) || `lark-cli exited ${result.code}`));
    return parsed;
}
...
function blocksFromXml(xml) {
    ...
    if (!starts.length)
        return [{ blockId: "document", topLevelIndex: 0, xml }];
...
function markerContent(xml) {
    return xml
        .replace(/^<p\b[^>]*>PENGUIN_API_DOC_BEGIN:[^<]*<\/p>/, "");
}
function newBlocks(envelope) { return Number(documentOf(envelope).revision_id ?? envelope.data?.revision_id ?? 0); }
export class LarkCliDocumentClient {
    runner;
    constructor(runner) {
        this.runner = runner;
    }
    async call(argv, stdin) {
        return jsonEnvelope(await this.runner.run([...argv, "--format", "json", "--as", "user"], stdin));
    }
    async fetchFull(nodeToken, revisionId) {
...
//# sourceMappingURL=lark-document-client.js.map