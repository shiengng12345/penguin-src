 ...
 *
 * The ten semantic/embedding table names below (semantic_chunks through
 * semantic_active_spaces) are no longer created by schema.ts on a fresh
 * install (the Nomic/ONNX embedding subsystem was removed), but this plan
 * deliberately never ran a DROP TABLE migration for them — an existing
 * install's database may still have them, populated with leftover data.
 * Keeping their names here (index-reset.ts's resetPredicate/
 * targetedFullResetPredicate already guard every table with tableExists()
 * before querying it, and the full-corpus reset path drops rebuildable
 * tables via DROP TABLE IF EXISTS) restores the user-invoked full-reset
 * pathway that can clear this leftover data, while remaining a safe no-op
 * on a fresh install where the tables never exist. */
