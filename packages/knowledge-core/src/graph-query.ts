...
import { graphEdgeEvidence } from "./graph-evidence.js";
...
const FRAMEWORK_EDGE_TYPES = new Set(["injects", "provides", "implements", "dispatches_to"]);

function graphEdgeMetadata(edgeType: unknown): Record<string, unknown> {
  const type = String(edgeType ?? "");
  return FRAMEWORK_EDGE_TYPES.has(type)
    ? { frameworkBoundary: true, frameworkEdgeType: type }
    : {};
}
...
export function graphQuery(store: KnowledgeStore, request: GraphQueryRequest): GraphQueryResult {
  ...
  if (request.scope?.snapshotId) {
    ...
    for (const edge of view.edges({ limit: 50000 })) {
      ...
      if (src) scopedEdges.push({ id: edge.id, src, dst, edge_type: edge.edgeType, status: "active", method: edge.method, provenance: JSON.stringify(edge.provenance), scope: edge.scope });
    }
  }
  ...
  while (queue.length > 0 && nodes.size < request.limit) {
    ...
    const edgeStatus = (value: unknown, method?: unknown, provenance?: unknown, edgeType?: unknown): "verified" | "candidate" => {
      const evidence = graphEdgeEvidence({ edgeType, method, provenance });
      return evidence.evidenceState !== "proven" || value === "suggested" || value === "candidate" ? "candidate" : "verified";
    };
    const rows = scopedEdges.length > 0
      ? scopedEdges.filter((edge) => (step.direction === "out" ? edge.src === item.id : step.direction === "in" ? edge.dst === item.id : edge.src === item.id || edge.dst === item.id) && (!step.edgeTypes.length || step.edgeTypes.includes(String(edge.edge_type))) && allowedStatuses.has(edgeStatus(edge.status, edge.method, edge.provenance, edge.edge_type))).map((edge) => ({ ...edge, status: edgeStatus(edge.status, edge.method, edge.provenance, edge.edge_type), next_id: step.direction === "out" ? edge.dst : step.direction === "in" ? edge.src : (edge.src === item.id ? edge.dst : edge.src) })).slice(0, request.limit)
      : store.db.prepare(`SELECT e.*, ${nextIdSql} AS next_id FROM edges e WHERE ${directionSql} ${typeSql} AND e.status <> 'rejected' AND (e.status='active' OR e.status='suggested' OR e.status IS NULL) LIMIT ?`).all(...nextIdParams, ...endpointParams, ...step.edgeTypes, request.limit) as Array<Record<string, unknown>>;
    for (const edge of rows) { const next = String(edge.next_id ?? ""); if (!next) continue; if (request.where?.pathPrefixes?.length && !Boolean(store.db.prepare(`SELECT 1 FROM symbol_versions WHERE node_id=? AND (${request.where.pathPrefixes.map(() => "file_path LIKE ?").join(" OR ")}) LIMIT 1`).get(next, ...request.where.pathPrefixes.map((prefix) => `${prefix.replace(/%/g, "\\%")}%`)))) continue; const path = [...item.path, next]; const row = edge as Record<string, unknown>; const evidence = graphEdgeEvidence({ edgeType: row.edge_type, origin: row.origin, method: row.method, confidence: row.confidence, provenance: row.provenance, evidenceId: row.evidence_id ?? row.evidenceId, scope: row.scope ?? (row.branch_id ? "revision" : undefined), branchId: row.branch_id }); const status: "verified" | "candidate" = row.query_status === "candidate" ? "candidate" : edgeStatus(row.status, row.method, row.provenance, row.edge_type); if (!allowedStatuses.has(status)) continue; const dispatchHop = row.edge_type === "dispatches_to"; edges.push({ ...row, ...graphEdgeMetadata(row.edge_type), status, graphEvidence: evidence, ...(dispatchHop ? { dispatchHop: true, hopType: "dispatch" } : {}) }); paths.push(path); if (item.depth + 1 >= step.minDepth) queue.push({ id: next, depth: item.depth + 1, path }); if (edges.length >= request.limit) break; }
  }
