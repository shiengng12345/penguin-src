...
export function graphQuery(store: KnowledgeStore, request: GraphQueryRequest): GraphQueryResult {
  ...
  while (queue.length > 0 && nodes.size < request.limit) {
    ...
    const edgeStatus = (value: unknown, method?: unknown): "verified" | "candidate" => value === "suggested" || value === "candidate" || method === "INFERRED" ? "candidate" : "verified";
    const rows = scopedEdges.length > 0
      ? scopedEdges.filter((edge) => (step.direction === "out" ? edge.src === item.id : step.direction === "in" ? edge.dst === item.id : edge.src === item.id || edge.dst === item.id) && (!step.edgeTypes.length || step.edgeTypes.includes(String(edge.edge_type))) && allowedStatuses.has(edgeStatus(edge.status, edge.method))).map((edge) => ({ ...edge, status: edgeStatus(edge.status, edge.method), next_id: step.direction === "out" ? edge.dst : step.direction === "in" ? edge.src : (edge.src === item.id ? edge.dst : edge.src) })).slice(0, request.limit)
    ...
    for (const edge of rows) { const next = String(edge.next_id ?? ""); if (!next) continue; if (request.where?.pathPrefixes?.length && !Boolean(store.db.prepare(`SELECT 1 FROM symbol_versions WHERE node_id=? AND (${request.where.pathPrefixes.map(() => "file_path LIKE ?").join(" OR ")}) LIMIT 1`).get(next, ...request.where.pathPrefixes.map((prefix) => `${prefix.replace(/%/g, "\\%")}%`)))) continue; const path = [...item.path, next]; const row = edge as Record<string, unknown>; edges.push({ ...row, status: row.query_status ?? row.status ?? "verified" }); paths.push(path); if (item.depth + 1 >= step.minDepth) queue.push({ id: next, depth: item.depth + 1, path }); if (edges.length >= request.limit) break; }
  }
