...
const STABLE_MCP_LAUNCHER: &str = include_str!("../../scripts/knowledge-mcp-launcher.mjs");

fn stable_mcp_launcher_path() -> Option<PathBuf> {
    dirs::home_dir().map(|home| home.join(".penguin/bin/penguin-mcp"))
}
...
fn install_stable_mcp_launcher() -> Result<PathBuf, String> {
    ...
    write_atomic(&script_path, STABLE_MCP_LAUNCHER)?;
    ...
    write_atomic(&launcher, wrapper)?;
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        for path in [&launcher, &script_path] {
...
fn is_stable_mcp_client_target(command: &str, server: &str) -> bool {
    stable_mcp_launcher_path()
        .map(|launcher| command == launcher.to_string_lossy() && server.is_empty())
        .unwrap_or(false)
}

fn stable_mcp_launcher_healthy() -> bool {
    let Some(launcher) = stable_mcp_launcher_path() else {
        return false;
    };
    let target = launcher
        .parent()
        .map(|dir| dir.join("penguin-mcp-launcher.mjs"));
    match target {
        Some(target) => is_executable_file(&launcher) && is_executable_file(&target),
        None => false,
    }
}
