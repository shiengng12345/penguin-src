...
fn install_launcher<R: tauri::Runtime>(app: &tauri::AppHandle<R>) -> Result<PathBuf, String> {
    ...
    let wasm_line = inv
        ...
        .unwrap_or_default();
    // Prefer the user's own `node` from PATH so the CLI follows the machine's
    // active runtime (nvm/asdf/homebrew switches included) — a pinned absolute
    // path goes stale the moment the user upgrades node, and native modules
    // (better-sqlite3) then fail with an ABI mismatch. The node detected at
    // install time remains the fallback for shells/GUIs with no node on PATH.
    let script = format!(
        "#!/bin/sh\n# Auto-generated by Penguin.app — runs the bundled knowledge CLI.\n# Uses the `node` on your PATH when available; falls back to the node\n# detected at install time.\n{}if command -v node >/dev/null 2>&1; then\n  exec node \"{}\" \"$@\"\nfi\nexec \"{}\" \"{}\" \"$@\"\n",
        wasm_line,
        inv.cli.display(),
        inv.node.display(),
        inv.cli.display(),
