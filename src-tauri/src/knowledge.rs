...
struct CliInvocation {
    ...
    // True when this invocation is the packaged self-contained runtime: its
    // node_modules (better-sqlite3 prebuild) match ONLY the vendored node, so
    // the launcher must pin that node — a PATH node of a different ABI fails
    // to load the native module. Dev invocations run the workspace dist
    // against the workspace's own node_modules, which track the developer's
    // node — there the launcher prefers PATH so nvm switches keep working.
    self_contained: bool,
}
...
const EXPECTED_CAPABILITY_HASH: &str = "612319c1e6048355cc469bb7ad1efb1a2b01afe34ea585897cbad9fd028e1d26";
...
fn bundled_invocation<R: tauri::Runtime>(app: &tauri::AppHandle<R>) -> Option<CliInvocation> {
    ...
    if node.exists() && cli.exists() {
        ensure_executable(&node);
        Some(CliInvocation { node, cli, wasm_dir: Some(dir.join("wasm")), self_contained: true })
    } else {
        None
    }
}
...
fn dev_invocation<R: tauri::Runtime>(app: &tauri::AppHandle<R>) -> Option<CliInvocation> {
    ...
    Some(CliInvocation { node: resolve_node()?, cli: cli?, wasm_dir: None, self_contained: false })
}
...
fn install_launcher<R: tauri::Runtime>(app: &tauri::AppHandle<R>) -> Result<PathBuf, String> {
    ...
    let wasm_line = launch_inv
        ...
        .unwrap_or_default();
    // Which node the wrapper runs depends on what the CLI target is:
    // - packaged self-contained runtime → PIN the vendored node. Its
    //   node_modules ship a better-sqlite3 prebuild for exactly that ABI; a
    //   PATH node of another major version fails to load the native module.
    // - dev workspace dist → prefer the PATH node (nvm/asdf/homebrew switches
