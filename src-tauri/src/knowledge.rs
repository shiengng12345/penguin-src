...
pub fn install_cli_command<R: tauri::Runtime>(app: tauri::AppHandle<R>) {
    std::thread::spawn(move || {
        let inv = match resolve_invocation(&app) {
            Ok(i) => i,
            Err(_) => return, // CLI not resolvable yet — nothing to link
        };
        let home = match std::env::var_os("HOME") {
            Some(h) => PathBuf::from(h),
            None => return, // no HOME (non-unix) — skip
        };
        ...
        // Rewrite only when content changed, to avoid needless disk churn.
        if std::fs::read_to_string(&target).ok().as_deref() != Some(script.as_str()) {
            if std::fs::write(&target, &script).is_err() {
                return;
            }
        }
        ensure_executable(&target);
        // Nudge if ~/.local/bin isn't on PATH (the launcher exists but won't resolve).
        if let Some(path) = std::env::var_os("PATH") {
            let on_path = std::env::split_paths(&path).any(|p| p == bin_dir);
            if !on_path {
                eprintln!(
                    "[pengvi] installed `penguin` at {} — add ~/.local/bin to your PATH to use it in a terminal",
                    target.display()
                );
            }
        }
