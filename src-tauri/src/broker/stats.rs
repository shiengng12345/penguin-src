...
//!
//! The three sentinel-carrying enums this module's fields rely on
//! (`BacklogAge`, `ConsumerTimestamp`, `SubscriptionType`) and the
//! normaliser functions that populate them from Pulsar's in-band magic
//! values live in `broker::stats_sentinels` (split out to keep both files
//! under this project's line cap) and are re-exported below, so every
//! existing `crate::broker::stats::{BacklogAge, ...}` path keeps working
//! unchanged. See that module's doc for the "Sentinel values" rationale.

use crate::broker::envelope::{BrokerError, BrokerErrorCode};
use crate::broker::stats_wire::RawTopicStats;
use serde::{Deserialize, Serialize};
pub use crate::broker::stats_sentinels::{BacklogAge, ConsumerTimestamp, SubscriptionType};
use crate::broker::stats_sentinels::{
    normalize_backlog_age, normalize_consumer_timestamp, normalize_subscription_type,
};
