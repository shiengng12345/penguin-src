<!-- generated by scripts/knowledge-docs-generate.mjs; edit the canonical manifest instead -->
<!-- capability-hash: f99fca378bb72ee30313ccc9da98ce4eb353c9b41eff230726186fd16446b67f -->

# Penguin Knowledge Schema Reference

- **Current schema version:** 18

## Migration metadata

| Version | Name | Summary |
| --- | --- | --- |
| 10 | `source-snapshots` | content-addressed source blobs, snapshot overlays, coverage and resolver metadata |
| 11 | `markdown-vault` | Markdown sections, properties, wikilinks, evidence and saved-query records |
| 12 | `semantic-and-memory` | memory, ontology, semantic chunks, embeddings and reflection records |
| 13 | `trust-and-external-sources` | validated findings, audit events, external sources and revision-safe evidence |
| 14 | `coverage-layers-and-edge-boundaries` | coverage_layers table; edges.evidence_id + edges.boundary; forced rebuild on schema bump |
| 15 | `canonical-endpoints-and-membership` | parser-derived endpoint aliases and explicit provider/consumer/declaration repository membership |
| 16 | `concrete-coverage-debt` | revision-scoped unresolved reference work queue alongside aggregate coverage |
| 17 | `semantic-identities-and-lifecycle` | provenance-complete chunk identities and atomic embedding space/generation/job lifecycle |
| 18 | `semantic-background-worker` | durable semantic worker leases, pause controls, and retry scheduling without changing parser-derived graph format |

## Tables

- `meta`
- `repos`
- `branches`
- `git_commits`
- `revision_snapshots`
- `deployment_revisions`
- `revision_references`
- `nodes`
- `node_aliases`
- `endpoint_aliases`
- `endpoint_memberships`
- `symbol_versions`
- `edges`
- `parser_edge_sets`
- `coverage_layers`
- `files_index`
- `events`
- `ledger_state`
- `workspaces`
- `workspace_repos`
- `notes_index`
- `entities`
- `response_samples`
- `credential_entries`
- `pending_frontend_edges`
- `fts_notes`
- `fts_symbols`
- `fts_symbol_rows`
- `fts_identifiers`
- `fts_identifier_rows`
- `file_facts`
- `file_fact_symbols`
- `snapshot_overlays`
- `effective_snapshot_files`
- `snapshot_rename_events`
- `resolution_sets`
- `resolved_edges`
- `snapshot_resolution_refs`
- `revision_endpoint_publications`
- `revision_endpoint_occurrences`
- `global_resolved_edges`
- `source_blobs`
- `source_blob_line_offsets`
- `source_blob_trigrams`
- `source_facts`
- `file_fact_sources`
- `effective_snapshot_sources`
- `source_snapshot_overlays`
- `source_backfill_checkpoints`
- `coverage_records`
- `unresolved_reference_coverage`
- `unresolved_reference_items`
- `markdown_sections`
- `note_properties`
- `note_links`
- `why_cards`
- `memory_items`
- `ontology_terms`
- `ontology_links`
- `saved_queries`
- `trust_evidence`
- `validated_findings`
- `finding_evidence`
- `knowledge_audit_events`
- `search_feedback`
- `semantic_chunks`
- `embedding_models`
- `semantic_embedding_refs`
- `semantic_vector_values`
- `embedding_spaces`
- `embedding_generations`
- `embedding_jobs`
- `semantic_worker_leases`
- `semantic_controls`
- `semantic_active_spaces`
- `reflection_suggestions`
- `external_knowledge_sources`
- `external_calls`
- `knowledge_gc_runs`
- `knowledge_size_samples`
- `source_path_fts`
