...
function observeOwnershipHandoffs(boundary) {
  ...
  for (const claimed of handoffClaims(boundary.registry)) {
    ...
    // A token only correlates a request. This launcher issues ownership in
    // memory after it verifies the concrete process is its live descendant.
...
try {
  ...
  child = spawn(runtime.node, [runtime.cli, ...process.argv.slice(2)], {
...
// Resolve macOS /var -> /private/var aliases before deciding whether this
// copied stable launcher is the process entry point.
let isMain = false;
try {
